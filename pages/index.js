...
function exportarCSV(registros) {
  ...
  const filas = registros.map((r,i) => {
    ...
    return [txt(String(i+1).padStart(3,"0")),txt(fecha(r.fecha||Date.now())),txt(r.contexto||r.localizacion||""),num(r.edad),txt(r.sexo||""),num(r.peso),num(r.fg),num(r.n_farmacos||r.medicamentos?.length||0),num(r.n_total_interacciones||inter.length),num(r.n_graves||r.counts?.grave||0),num(r.n_moderadas||r.counts?.moderada||0),num(r.n_leves||r.counts?.leve||0),num(r.n_frecuentes||inter.filter(x=>x.frecuencia==="frecuente").length),num(r.n_ocasionales||inter.filter(x=>x.frecuencia==="ocasional").length),num(r.n_raras||inter.filter(x=>x.frecuencia==="rara").length),txt(r.grave_frecuente_1||gf[0]?`${gf[0]?.farmaco1} + ${gf[0]?.farmaco2}`:""),txt(r.grave_frecuente_2||gf[1]?`${gf[1]?.farmaco1} + ${gf[1]?.farmaco2}`:""),txt(r.grave_frecuente_3||gf[2]?`${gf[2]?.farmaco1} + ${gf[2]?.farmaco2}`:""),txt(r.grave_ocasional_1||go[0]?`${go[0]?.farmaco1} + ${go[0]?.farmaco2}`:""),txt(r.grave_ocasional_2||go[1]?`${go[1]?.farmaco1} + ${go[1]?.farmaco2}`:""),txt(r.grave_ocasional_3||go[2]?`${go[2]?.farmaco1} + ${go[2]?.farmaco2}`:""),txt(r.cambio_medicacion||""),txt(r.comentarios||"")].join(S);
  });
...
export default function App() {
    ...
    try {
      ...
      for(const img of [imagen1,imagen2].filter(Boolean)) {
        const res = await fetch("/api/analyze", {
          ...
          body: JSON.stringify({
            prompt: "Extrae TODOS los medicamentos de esta imagen sin excepcion. Devuelve UNICAMENTE un array JSON (sin markdown): [{\"nombre\":\"\",\"dosis\":\"\",\"frecuencia\":\"\",\"via\":\"\"}]. Solo el JSON.",
            imagen: img.data, mediaType: img.mediaType
          })
        });
        const data = await res.json();
      ...
      if(filtrados.length>0){ setMeds(filtrados); setImagen1(null); setImagen2(null); setPreview1(null); setPreview2(null); }
      else setError("No se detectaron medicamentos. Intenta con otra foto.");
    } catch(e) { setError("Error al procesar la imagen."); }
    setExtrayendo(false);
  };
  ...
  const exportarPDF = () => {
    ...
    const interHtml = ["grave","moderada","leve"].map(nivel=>{
      ...
      const label={grave:"GRAVE",moderada:"MODERADA",leve:"LEVE"};
      return `<h3 style="color:${c[nivel]};font-size:12px;margin:14px 0 6px;">${label[nivel]}</h3>`
        ...
        <p style="font-size:11px;margin:0;color:#333"><strong>Recomendacion:</strong> ${i.recomendacion}</p>
    ...
    h1{font-size:17px;color:#185FA5;}table{width:100%;border-collapse:collapse;margin:8px 0;}
    ...
    <p style="color:#888;font-size:10px;margin:0 0 14px;">${fecha}${contexto?" · "+contexto:""}</p>
    <table><tr><th>Edad</th><th>Sexo</th><th>Peso</th><th>FG</th><th>Condicionantes</th></tr>
    <tr><td>${edad||"—"}</td><td>${sexo||"—"}</td><td>${peso?peso+" kg":"—"}</td><td>${fg?fg+" mL/min":"—"}</td><td>${otros||"—"}</td></tr></table>
    ...
    <p style="font-size:11px;line-height:1.6;">${resultado.resumen}</p>
    ${interHtml}
          ...
          <>
            ...
            <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:14,padding:"1.25rem",marginBottom:"1rem"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",gap:10,marginBottom:10}}>
                <div><label style={{fontSize:11,fontWeight:600,color:"#555",display:"block",marginBottom:5}}>EDAD *</label><input type="number" value={edad} onChange={e=>setEdad(e.target.value)} style={{width:"100%",boxSizing:"border-box",border:"1px solid #ddd",borderRadius:8,padding:"9px 12px",fontSize:13,outline:"none"}}/></div>
                ...
                <div><label style={{fontSize:11,fontWeight:600,color:"#555",display:"block",marginBottom:5}}>PESO</label><input type="number" value={peso} onChange={e=>setPeso(e.target.value)} style={{width:"100%",boxSizing:"border-box",border:"1px solid #ddd",borderRadius:8,padding:"9px 12px",fontSize:13,outline:"none"}}/></div>
                <div><label style={{fontSize:11,fontWeight:600,color:"#555",display:"block",marginBottom:5}}>FG</label><input type="number" value={fg} onChange={e=>setFg(e.target.value)} style={{width:"100%",boxSizing:"border-box",border:"1px solid #ddd",borderRadius:8,padding:"9px 12px",fontSize:13,outline:"none"}}/></div>
              </div>
              <div><label style={{fontSize:11,fontWeight:600,color:"#555",display:"block",marginBottom:5}}>OTROS CONDICIONANTES</label><input type="text" value={otros} onChange={e=>setOtros(e.target.value)} style={{width:"100%",boxSizing:"border-box",border:"1px solid #ddd",borderRadius:8,padding:"9px 12px",fontSize:13,outline:"none"}}/></div>
            </div>
            <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:14,padding:"1.25rem",marginBottom:"1rem"}}>
              ...
              {meds.map((m,i)=>(
                <div key={i} style={{background:"#f8f9fb",borderRadius:10,padding:"12px",marginBottom:8,border:"1px solid #e5e7eb"}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",gap:8}}>
                    ...
                    <div><label style={{fontSize:11,fontWeight:600,color:"#555",display:"block",marginBottom:4}}>DOSIS</label><input value={m.dosis} onChange={e=>upd(i,"dosis",e.target.value)} style={{width:"100%",boxSizing:"border-box",border:"1px solid #ddd",borderRadius:8,padding:"9px 12px",fontSize:13,outline:"none"}}/></div>
                    <div><label style={{fontSize:11,fontWeight:600,color:"#555",display:"block",marginBottom:4}}>FRECUENCIA</label><input value={m.frecuencia} onChange={e=>upd(i,"frecuencia",e.target.value)} style={{width:"100%",boxSizing:"border-box",border:"1px solid #ddd",borderRadius:8,padding:"9px 12px",fontSize:13,outline:"none"}}/></div>
                    <div><label style={{fontSize:11,fontWeight:600,color:"#555",display:"block",marginBottom:4}}>VIA</label><input value={m.via} onChange={e=>upd(i,"via",e.target.value)} style={{width:"100%",boxSizing:"border-box",border:"1px solid #ddd",borderRadius:8,padding:"9px 12px",fontSize:13,outline:"none"}}/></div>
                  </div>
                </div>
              ))}
              ...
              <div style={{borderTop:"1px solid #e5e7eb",paddingTop:12}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                  {[[preview1,"Imagen 1",e=>handleImg(e,1),()=>{setImagen1(null);setPreview1(null);}],[preview2,"Imagen 2",e=>handleImg(e,2),()=>{setImagen2(null);setPreview2(null);}]].map(([preview,label,handler,clear],idx)=>(
                    <div key={idx}>
                      {!preview?(
                ...
                {(imagen1||imagen2)&&(
                  <button onClick={extraerMedicamentos} disabled={extrayendo} style={{background:extrayendo?"#B5D4F4":"#185FA5",color:"#fff",border:"none",borderRadius:8,padding:"9px 0",fontSize:13,cursor:"pointer",width:"100%",marginTop:8}}>
                    {extrayendo?"Extrayendo...":"✨ Extraer medicamentos"}
                  </button>
                )}
              </div>
            </div>
            ...
            {registros.length===0?(
              ...
              <>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:10,marginBottom:"1rem"}}>
                  {[["Analisis",registros.length,"#185FA5","#E6F1FB"],["Interacciones",registros.reduce((a,r)=>a+(r.interacciones?.length||r.n_total_interacciones||0),0),"#791F1F","#FCEBEB"],["Edad media",Math.round(registros.filter(r=>r.edad).reduce((a,r)=>a+(r.edad||0),0)/Math.max(registros.filter(r=>r.edad).length,1))||0,"#27500A","#EAF3DE"]].map(([l,v,txt,bg])=>(
                ...
                <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:12,padding:"1rem",marginBottom:"1rem"}}>
                  ...
                  {["grave","moderada","leve"].map(n=>{
                    const total=registros.reduce((a,r)=>a+(r.interacciones?.length||r.n_total_interacciones||0),0);
                    const v=registros.reduce((a,r)=>a+(r.counts?.[n]||r[`n_${n}s`]||0),0);
                  ...
                  {registros.slice(0,10).map((r,i)=>(
                    <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:i<9?"1px solid #f0f0f0":"none"}}>
                      <div style={{flex:1}}>
                        ...
                        <p style={{fontSize:11,color:"#888",margin:0}}>{r.edad}{r.sexo?` · ${r.sexo}`:""}{r.contexto||r.localizacion?` · ${r.contexto||r.localizacion}`:""}</p>
                      </div>
                      <div style={{display:"flex",gap:4}}>
                        {["grave","moderada","leve"].map(n=>{
                          const v=r.counts?.[n]||r[`n_${n}s`]||0;
                          return v>0&&(<span key={n} style={{fontSize:10,fontWeight:600,padding:"2px 6px",borderRadius:10,background:NIVEL[n].badge,color:NIVEL[n].badgeText}}>{v}{n[0].toUpperCase()}</span>);
                        })}
                      </div>
                    </div>
                  ))}
          ...
          <div>
            {!adminOk?(
              <div style={{maxWidth:320,margin:"3rem auto",textAlign:"center"}}>
                ...
                <input type="password" value={adminPass} onChange={e=>{setAdminPass(e.target.value);setAdminError(false);}} onKeyDown={e=>{if(e.key==="Enter"){if(adminPass==="Medicina2026")setAdminOk(true);else setAdminError(true);}}} placeholder="" style={{width:"100%",boxSizing:"border-box",border:`1px solid ${adminError?"#E24B4A":"#ddd"}`,borderRadius:8,padding:"10px 14px",fontSize:14,outline:"none",marginBottom:8,textAlign:"center"}}/>
              ...
              <div>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1rem",flexWrap:"wrap",gap:8}}>
                  <div>
                    ...
                    <p style={{fontSize:12,color:"#888",margin:0}}>{registros.length} registro{registros.length!==1?"s":""}</p>
                  </div>
                ...
                {registros.length===0?(
                  <div style={{textAlign:"center",padding:"3rem",color:"#888"}}><div style={{fontSize:36,marginBottom:10}}>📭</div><p>No hay registros.</p></div>
                ):registros.map((r,i)=>{
                  const fd=new Date(r.created_at||r.fecha);
                  const fecha=r.fecha_str||`${String(fd.getDate()).padStart(2,"0")}/${String(fd.getMonth()+1).padStart(2,"0")}/${String(fd.getFullYear()).slice(2)}`;
                  return(
                      ...
                      <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
                        <div style={{flex:1}}>
                          <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:4}}>
                            {(r.contexto||r.localizacion)&&<span style={{fontSize:10,fontWeight:600,padding:"2px 7px",borderRadius:10,background:"#E6F1FB",color:"#185FA5"}}>{r.contexto||r.localizacion}</span>}
                            {["grave","moderada","leve"].map(n=>{
                              const v=r.counts?.[n]||r[`n_${n}s`]||0;
                              return v>0&&(<span key={n} style={{fontSize:10,fontWeight:600,padding:"2px 7px",borderRadius:10,background:NIVEL[n].badge,color:NIVEL[n].badgeText}}>{v} {LABEL[n].toLowerCase()}{v>1?"s":""}</span>);
                            })}
                          </div>
                        ...
                        {confirmDelete===i?(
                          <div style={{display:"flex",flexDirection:"column",gap:4,alignItems:"flex-end"}}>
                            ...
                            <div style={{display:"flex",gap:6}}>
                              <button onClick={()=>eliminarRegistro(i,r.id)} style={{fontSize:11,padding:"4px 10px",background:"#E24B4A",color:"#fff",border:"none",borderRadius:6,cursor:"pointer"}}>Eliminar</button>
                        ...
                        ):<button onClick={()=>setConfirmDelete(i)} style={{fontSize:11,padding:"5px 10px",background:"#FCEBEB",color:"#A32D2D",border:"1px solid #F7C1C1",borderRadius:8,cursor:"pointer"}}>🗑 Eliminar</button>}
                      </div>
                    </div>
                  );
                })}
